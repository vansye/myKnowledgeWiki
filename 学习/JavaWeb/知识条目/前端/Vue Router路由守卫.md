...
### 概念
路由守卫是 Vue Router 在导航不同阶段触发的钩子函数，`beforeEach` 是最常用的全局前置守卫，在每次路由跳转前触发，可用于权限检查、数据预加载等。与 [[Vue Router路由配置]] 中的路由定义配合使用，共同实现 SPA 的导航控制。

### 重点
- **beforeEach**：全局前置守卫，接收 `(to, from, next)` 三个参数，`next()` 继续导航，`next('/path')` 重定向
- **权限校验**：检查 `localStorage.token`，未登录且非登录页时跳转登录页
- **多层防护**：与 [[API接口封装]] 中的响应拦截器配合，形成路由层 + API 层的双重防护
